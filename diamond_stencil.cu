...
__global__ void compute_vn_and_reduce_dvt(
    ...
    const float2* __restrict__ uv, const float2* __restrict__ primal_normal_vert,
    const float2* __restrict__ dual_normal_vert, dawn::float_type* __restrict__ vn_vert) {
...
void DiamondStencil::diamond_stencil::run() {
  ...
  compute_vn_and_reduce_dvt<<<dG, dB>>>(mesh_.NumEdges(), mesh_.NumNodes(), kSize_,
                                        mesh_.ECVTable(), dvt_tang_, dvt_norm_, uv_,
                                        primal_normal_vert_, dual_normal_vert_, vn_vert_);
